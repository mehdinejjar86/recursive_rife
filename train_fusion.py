...
with open(metrics_file_path, 'w', newline='') as csvfile:
    ...
    writer.writerow(['Epoch', 'LR', 'Train_Loss', 'Train_SSIM_Score', 'Train_MSE_Loss', 'Train_PSNR', 'Val_Loss', 'Val_SSIM_Score', 'Val_MSE_Loss', 'Val_PSNR'])
...
for epoch in range(num_epochs):
    ...
    for i, (I0, I1, flows, masks, timestep, I_gt) in enumerate(loop):
        ...
        loop.set_postfix(loss=total_loss.item(), ssim=ssim_score.item(), mse=mse_loss.item(), psnr=psnr_score.item())
    ...
    with torch.no_grad():
        ...
        for i, (I0, I1, flows, masks, timestep, I_gt) in enumerate(val_loop):
            ...
            val_loop.set_postfix(loss=total_loss.item(), ssim=ssim_score.item(), mse=mse_loss.item(), psnr=psnr_score.item())
    ...
    print(f"Epoch [{epoch + 1}/{num_epochs}], LR: {current_lr:.6f}, Train Loss: {avg_train_loss:.4f}, Train SSIM: {avg_train_ssim_score:.4f}, Train MSE: {avg_train_mse_loss:.4f}, Train PSNR: {avg_train_psnr:.4f}, Val Loss: {avg_val_loss:.4f}, Val SSIM: {avg_val_ssim_score:.4f}, Val MSE: {avg_val_mse_loss:.4f}, Val PSNR: {avg_val_psnr:.4f}")

    with open(metrics_file_path, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([epoch + 1, current_lr, avg_train_loss, avg_train_ssim_score, avg_train_mse_loss, avg_train_psnr, avg_val_loss, avg_val_ssim_score, avg_val_mse_loss, avg_val_psnr])
